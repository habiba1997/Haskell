...
-- func _ (Just a) = a
-- main = do
--     putStrLn (func 1 (Just 0))
--     putStrLn (func 2 (Just 5))
--     putStrLn (func 0 (Nothing))
...
-- ---------- LISTS ----------
-- Lists are singly linked and you can only add to the front of 	
...
User
-- Calculate the Fibonacci Sequence
-- 1, 1, 2, 3, 5, 8, ...-- 1 : 1 : says to add 2 1s to the beginning of a listfrib :: a -> [a]
frib 1 = [1]
frib 2 =   [1,1]
frib n = [1,1] ++ [(frib(n-1) !! n-2) + ((frib(n-2) !! n-3))]
